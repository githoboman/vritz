...
export const IDENTITY_DERIVATION_VERSION = "Vritz-identity-v1";
...
export function identityDerivationMessage(accountHex: string, assetId: string): string {
  return [
    `${IDENTITY_DERIVATION_VERSION} — derive Vritz identity secret`,
    `asset: ${assetId}`,
    `account: ${accountHex}`,
    "Only sign this in the Vritz app. Never share the resulting signature.",
  ].join("\n");
}
...
export async function deriveIdentity(signatureHex: string): Promise<DerivedIdentity> {
  ...
  const identitySecret = sigToField(sigBytes, "Vritz::identity/v1");
  const salt = sigToField(sigBytes, "Vritz::salt/v1");

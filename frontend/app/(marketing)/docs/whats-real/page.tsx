...
const LIVE: readonly [string, string][] = [
  ["Six contracts on botchain-testnet", "locked packages (registry, challenge, verifier, both filters, token) + the EVM token — install txs in DEPLOYMENT.md"],
  ["Recipient-aware EVM transfer gating", "real reverts (user error 159) for sanctioned sender, ineligible recipient, and post-fraud holder — clickable on Verify"],
  ["On-chain Groth16 verification in the challenge path", "the fraud slash tx consumed ~80 BOT running the pairing check inside the contract"],
  ["Economic slash", "500 BOT slashed, 640 to the challenger, 110 transferred to the treasury account (spendable — a treasury transfer, not a burn)"],
...
const DEMO: readonly [string, string][] = [
  ...
  ["Sanctions mapping", "the live OFAC list holds ETH addresses; a labeled demo denylist is used for demo accounts"],
  ["The officer role", "a single demo key, not a multisig (multisig is the documented production path)"],
...
const LIMITS: readonly string[] = [
  ...
  "The EVM contract is upgradable by the installer key (the compliance logic it calls — registry + filter — is locked); a production deployment would lock it or move the upgrade URef to a multisig.",
...
export default function DocsWhatsReal() {
  return (
    ...
    >
      <h3 className="font-semibold text-ink">Live on BOT Chain testnet (verify each on the next page)</h3>

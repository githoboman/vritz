...
const COMPARE = [
  {
    ...
    Vritz: "Never custodied. Eligibility proven in zero-knowledge",
  },
  {
    point: "Screening",
    usual: "Point-in-time KYC: pass once, in forever",
    Vritz: "Screened against live sanctions data at onboarding and refresh; revocable on-chain at any time",
  },
  {
    point: "Enforcement",
  ...
  return (
    <ScrollFx>
        ...
        <Container className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
          <div data-reveal>
            ...
            <h2 className="mt-4 font-serif text-3xl font-medium tracking-[-0.015em] text-ink sm:text-4xl">
              A sanctioned wallet tries to receive the bond. Vritz blocks it on-chain. Nobody ever
              saw their name.
            </h2>
            ...
            <p className="mt-4 text-sm text-ink-subtle">
              A walkthrough of the real mechanism — the same recipient-aware transfer filter that {" "}
              <Link href="https://scan.botchain.ai" target="_blank" rel="noreferrer" className="text-brand underline-offset-2 hover:underline">
                reverts a sanctioned transfer live on BOT Chain testnet
              </Link>
              . Press the button.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 divide-y divide-border sm:grid-cols-3 sm:divide-x sm:divide-y-0">
              ...
              {COMPARE.map((row) => (
                <div key={row.point} className="flex flex-col gap-4 p-6">
                  ...
                  <p className="text-sm font-medium leading-relaxed text-ink">{row.Vritz}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <Container>
          <div data-reveal>
            ...
            <p className="mt-4 max-w-2xl text-[15px] leading-relaxed text-ink-muted">
              Vritz doesn&apos;t pick a vertical. It owns the job every vertical has to solve.
            </p>
          </div>
        ...
        <Container className="max-w-3xl">
          ...
          <Card data-reveal className="mt-8 p-7">
            <p className="text-[15px] leading-relaxed text-ink">
              Vritz uses an honest on-chain/off-chain split: proofs are generated in the browser
              and verified off-chain before attestation; BOT Chain stores the signed credential —
              commitment, nullifier, expiry, and the holder&apos;s own proof bytes — and enforces
              transfer eligibility through a recipient-aware EVM filter. The challenge path
              re-verifies the published proof on-chain (Groth16) to adjudicate disputes.
            </p>
          </Card>
      ...
      <section className="border-b border-border bg-surface py-16 sm:py-24">
        <Container className="max-w-3xl">
          <div data-reveal>
            <Eyebrow>Built on BOT Chain</Eyebrow>
            <h2 className="mt-4 font-serif text-3xl font-medium tracking-[-0.015em] text-ink sm:text-4xl">
              High-performance compliance, native to BOT Chain.
            </h2>
          </div>
          <p data-reveal className="mt-6 text-[15px] leading-relaxed text-ink-muted">
            BOT Chain is an EVM-compatible Layer 1 designed for AI Agent, DePIN, and verifiable computation. Vritz brings an institutional security-token model — identity, claims, compliance rules, transfer restrictions — to BOT Chain&apos;s native high-performance ecosystem, built to pay for live compliance data on demand. It sits dead-center on BOT Chain&apos;s intelligent economy thesis.
          </p>
        </Container>
      </section>

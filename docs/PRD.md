...
**Privacy-preserving, agent-operated compliance for tokenized real-world assets on BOT Chain.**

`v1.0` · BOT Chain Agentic Buildathon 2026 · BOT Chain Innovation Track · Submission deadline June 30, 2026
...
Demo reference asset: a **tokenized corporate bond** on BOT Chain testnet (one concrete, real instance — not a mock; horizontal product, single reference deployment).
...
The agent is the spine (perceive → decide → act); ZK is the privacy mechanism; the on-chain ERC-721 filter is the enforcement teeth.
...
**Vs. the obvious ERC-3643 / T-REX playbook.** A vanilla compliant-token implementation gates transfers against a public identity registry. Writ adds the three things that playbook lacks: **privacy** (no PII custody, eligibility proven in ZK), **continuity** (autonomous runtime re-screening, not point-in-time), and an **autonomous operator** (the agent runs it). Writ is effectively *the ERC-3643-equivalent for BOT Chain, shipped ahead of the protocol* (BOT Chain's native compliance/identity stack is 2026 H2 roadmap; its compliant-privacy stack is 2027).

**Vs. zk-KYC prior art.** zk-identity/credential schemes exist in the wider market, but they are overwhelmingly **point-in-time credential issuance**. Writ is **runtime** (the agent re-evaluates the whole holder base against changing rules), **no-custody** (the verifier never sees PII either), **on-chain-enforced** (a real ERC-721 transfer filter, not just a credential), and **agent-operated**. That synthesis is the wedge.

**The ERC-721 gap.** BOT Chain's NFT standard (ERC-721) is *recipient-blind* on transfer — its filter hook can't see the recipient (proven empirically, §6). Writ's compliance token is a patched ERC-721 that *can* gate recipients. That's a concrete gap in BOT Chain's standards that Writ fills.

---

## 5. Architecture

**Keystone constraint (proven):** on-chain SNARK verification on BOT Chain is not cost-viable. There is no native EC-pairing host function on live testnet (`vm_botchain_v2 = false`); a pure-wasm Groth16 verify costs ~1000 BOT / ~62% of a block. Per-transfer on-chain verification is infeasible. **Therefore: verify off-chain, commit on-chain.**

```
OFF-CHAIN                                          ON-CHAIN (botchain-test, vm_botchain_v1)
...
Investor ──proof + public inputs──┐
 (PII + claims from KYC issuer)    │              ┌──────────────────────────────┐
                          ...
                          └────────┬─────────┘                    ▼
                                   │ fetch SDN        ┌──────────────────────────────┐
                    OFAC SDN (live)                   │ Writ Token (patched ERC-721,  │
...
- **On-chain:** the Writ Token (patched ERC-721), the Credential Registry (commitments, flags, expiries, quorum signatures, published proofs, bonds, nullifier set), RBAC, officer override authority.
...
Two complementary controls — ERC-721's filter fires on transfer but **not on mint** (proven), so issuance needs its own gate.
...
- Patched 441KB ERC-721 wasm installs live (557 BOT, no error) — closes "live deploy unverified."
...
- Per-transfer cost ≈ 0.87 BOT; the filter read itself is negligible.

**Writ's token = patched ERC-721 fork** (recipient read moved before the filter call; real recipient passed as `ARG_TARGET_KEY`). Forking the base token to add transfer restrictions is the security-token pattern — exactly what ERC-3643/T-REX do to ERC-20.
...
- **Capability demo:** ship a one-off on-chain Groth16 verify (Shroud-pattern, free testnet BOT) to prove on-chain verification *is possible*, then run the off-chain path for the real loop — honest about the cost tradeoff rather than hiding it.

**The exact honest claim — never deviate:** *"Eligibility proven in zero-knowledge, verified by a threshold of autonomous agents; the chain stores signed credentials, enforces compliance at every transfer via a native ERC-721 filter, and exposes published proofs for on-chain fraud challenge, with off-chain selective disclosure to regulators."* **Never** "on-chain SNARK verification."
...
- **ERC-3643 / T-REX semantic alignment** — Writ speaks the institutional security-token model (identity + claims + compliance rules + transfer restrictions). BOT Chain's native version is roadmap; Writ ships the equivalent now. Not a snowflake.
- **Bounded autonomy + human override** — the agent acts autonomously within its mandate; the compliance officer holds a high-weight BOT Chain key (behind M-of-N human multisig) that can freeze, override, or force-disclose. Uses BOT Chain's native weighted-action-threshold model — the protocol-level differentiator — and answers the "can a human stop it?" question every CCO asks, without undercutting "agentic" (bounded autonomy is more sophisticated than no-human autonomy).
...
- **OFAC / OpenSanctions SDN** — the live data source is the real OFAC SDN digital-currency list (free, authoritative, ecosystem-neutral). Honest scope (see README §3): the list holds ETH addresses, so it is screened against a holder-linked ETH address; BOT Chain-account matching uses a labeled demo denylist, since no official BOT Chain-account SDN mapping exists.
- **BOT.cloud MCP** — chain reads, balances, contract state for the agent (full testnet parity).

x402-paid OFAC was evaluated and dropped: real x402 OFAC services settle in USDC on Base/Solana, and no OFAC service sits behind the BOT Chain x402 facilitator, so an autonomous WBOT-settled OFAC screen on BOT Chain isn't a reachable endpoint. Screening goes directly against the authoritative SDN list rather than contorting the product around x402. (If x402 is wanted for BOT Chain-alignment, the clean path is metering Writ itself — issuers pay WBOT per compliance check — not OFAC-via-x402.)
...
1. **Writ Token** — finalize the patched ERC-721 fork (recipient-aware filter wired to read the Registry; mint gate). *Substrate already proven.*
2. **Credential Registry (Solidity)** — credential storage (commit/nullifier/flag/expiry/sigs), native `verify_signature` check of quorum sigs, RBAC, nullifier set, published-proof storage + pruning.
...
6. **Officer override** — BOT Chain weighted-multisig authority for freeze/override/disclose.
...
- **Use `put-deploy`, not `put-transaction`.** botchain-client 5.0.1 TransactionV1 mis-delivers `Option<Key>`/`Key` named args to `vm_botchain_v1` contracts (cep78 install reverted 156). Legacy `put-deploy` with a plain `Key` arg works. All contract calls use `put-deploy` until the TransactionV1 Key-arg issue is resolved.
- **Keep `--payment-amount` tight.** `payment_limited` pricing holds the full payment up front with delayed (~24h) gas-hold release; a failed 500 BOT install cost ~125 BOT net. Entrypoint calls are <1 BOT; budget accordingly.
- **Build on the patched erc-721 fork**, pinned `nightly-2025-02-04` (`-Z build-std`, `-C target-cpu=mvp`). 441KB wasm fits the `install_upgrade_lane` (max 750KB, 1000 BOT, 1/block).
- **Faucet is browser/CAPTCHA only** — fund keys manually.
- `vm_botchain_v2 = false` on testnet — BN254 pairing host functions remain unreachable (re-confirms the off-chain-verify architecture).

---

## 18. Tech stack

Contracts: **Solidity 2.8.x** + **patched ERC-721** (BOT Chain 2.0, `vm_botchain_v1`). ZK: **Circom v2 / snarkjs / Groth16-BN254** prover, **arkworks** verifier. Agent: **Railway**, MCP-driven (BOT.cloud), OFAC SDN sanctions screening. Infra: botchain-client 5.0.1 (`put-deploy`), BOT.cloud (REST/streaming/Node-RPC, testnet parity), testnet.cspr.live explorer.
...
- **Community vote (BOT.fans):** top 3 advance without judging. Lead with the one-sentence hook (§0). Mobilize the build-in-public audience to earn fan points and vote.
- **Merit (jury + Association discretionary advancement):** institutional depth, real-world applicability, dead-center on BOT Chain's RWA + compliant-privacy + agent thesis.

**Final-round criteria mapping:** Technical execution (real ZK + patched on-chain enforcement, proven live) · Innovation (only ZK-compliance entry in the field) · Agentic systems (autonomous runtime compliance officer as the spine) · Real-world applicability (the #1 RWA blocker) · UX (issuer/regulator dashboards) · Working smart contracts (live testnet, real txs) · Long-term plans (fundable B2B; ERC-3643-for-BOT Chain) · Long-term impact (the compliance primitive every RWA vertical needs).
...
- [x] Transaction-producing on-chain component on BOT Chain Testnet — real txs captured (install, mint, gated transfer, fail-safe deny).
- [ ] Open-source GitHub repo + README (documentation + usage).
- [ ] Public demo video (the §15 screenplay).
- [ ] BOT.fans listing + community mobilization.
...
## Live v4 proof points (BOT Chain testnet — copy-paste)
...
- Fraudulent attestation slashed, 110 BOT transferred to the treasury account (fraud slash):

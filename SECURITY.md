...
BOT Chain. This document describes what is in scope, the trust model, and how to report
a vulnerability responsibly.

## Scope & status

- **This is a testnet demo.** All contracts are deployed on **botchain-test** (BOT Chain 2.x).
...
- **Onboarding is verified off-chain.** The investor's identity secret is derived from
  ...
  list for a linked address; labeled demo denylist for BOT Chain accounts; fail-closed on
...
- **On-chain Groth16 verification runs only in the fraud-challenge path.** A challenger
  bonds BOT, disputes a credential, and `challenge.resolve` cross-calls the on-chain
...
- **Security holds** as long as the attestation operator is honest **or** any single
  ...
  trust model).
- **Enforcement is fail-safe deny.** Every ERC-721 transfer calls a recipient-aware
...
- No automatic BOT Chain-account OFAC detection — the live OFAC list holds ETH
  addresses (screened against a linked ETH address); BOT Chain-account matching is a
  labeled demo denylist.
- No officer multisig in the deployed demo — a single officer key (BOT Chain

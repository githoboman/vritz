...
Installed via `scripts/deploy/deploy_v4.py` (put-deploy; Solidity packages installed with
...
| contract | package hash (stable address) | contract hash | install tx | consumed (BOT) |
| --- | --- | --- | --- | --- |
...
| writ_registry_filter (ERC-721 hook) | `d84a932187624c1c982ed5c6dcbd1961fe370f732ce02fcbc0fe3e5e28389726` | `a2cc8c3d…bfbdaf933` | [`a738e2ba…`](https://testnet.cspr.live/deploy/a738e2bacd35ff9899943d5c66a5556d48f3c1d272af78b5239ee76b08b6267b) | 88.2 |
| transfer-filter (Solidity, writ-token) | `406e90f7646576e2eb252fe1ce5144823c12b09bfe4c21cede18f9333c5f6d8e` | `957deb64…460727e` | [`cecb0519…`](https://testnet.cspr.live/deploy/cecb05196c3b764d493eeb0698a72209262a4cef8ebcc0d0954d21c0b5778bd2) | 225.8 |
...
| writ-token (Solidity demo token) | `512068de722212ce497cb081049649339f0a8994394328164f3dde52c4ab8a3e` | `18eb2187…8761f13` | [`a39e60dc…`](https://testnet.cspr.live/deploy/a39e60dc52aff4f0c98550e31a3babbc235f52aa47c98dee9450cfcfb9d6e53c) | 225.6 |

Two filters exist by design: `writ_registry_filter` is the **production ERC-721 hook**
(raw BOT Chain contract from the fork tree, `can_transfer` + `mint_allowed`, wired into
writ-cep78 at install); `transfer-filter` is the Solidity adapter used by the Solidity
...
`account-hash-50f4e6e8…c1058bf6`, attestor_bond = 250 BOT (demo sizing).
...
The 110 BOT remainder of the slashed pool goes to the **treasury account**
(`account-hash-50f4e6e85014d95f48678abbdb27c89b78da7119ec1f857bee562f24c1058bf6`),
a normal spendable BOT Chain account configured at challenge install. It is **not
destroyed and not unspendable**. All docs and UI call this a *treasury transfer*.
(BOT Chain has no canonical zero-address sink for native BOT; making the remainder
provably unspendable is production roadmap.)

## The payable workaround (how the live slash was executed)

Solidity `#[payable]` entrypoints (`challenge.bond`, `challenge.challenge`) fund the
attached BOT by transferring from the caller's main purse inside a session. On this
BOT Chain 2.0 testnet node a legacy put-deploy session cannot spend from the main purse
...
- Solidity packages (verifier, registry, challenge, both filters, token) were installed
...
- **writ-cep78 is an upgradable package** (standard ERC-721 `new_contract` install);
  ...
  locked ERC-721 package or transfer the access URef to a multisig. Disclosed in
  README "Trust model".
...
- Committed wasm artifacts: `contracts/credential-registry/wasm/CredentialRegistry.wasm`
  ...
  clean clone without the Solidity build toolchain. Rebuild with `cargo odra build` in the
